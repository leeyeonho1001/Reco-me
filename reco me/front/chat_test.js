...
class ChatBot {
  ...
  async loadHistory() {
      try {
          const response = await fetch(`http://localhost:8000/history/${this.sessionId}`);
  ...
  async startNewChat() {
      try {
          // 현재 세션의 대화 기록 삭제
          await fetch(`http://localhost:8000/history/${this.sessionId}`, {
              method: 'DELETE'
          });
  ...
  async sendMessageToRecommend(message) {
    try { 
      ...
      const data = await response.json();
      this.addRecomends(data.response);
